...
commonLib.deliverAddressList.popup = {
    /**
     * 레이어 팝업 열기
     *
     * @param url : 팝업으로 열 주소
...
/* 레이어 팝업 요소 생성 S */
    ...
    layerDimEl.id = "deliveryAddressList_popup"
...
/* 레이어 팝업 가운데 배치 좌표 구하기 E */
    ...
    const layerDimEl = document.createElement("div");
layerDimEl.id = "deliveryAddressList_popup";
...
layerDimEl.addEventListener("click", this.close);
     ...
     */
close() {
    const popupEl = document.getElementById("deliveryAddressList_popup");
    if (popupEl) popupEl.parentElement.removeChild(popupEl);

    const layerDimEl = document.getElementById("deliveryAddressList_popup");
